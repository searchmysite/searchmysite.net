...
def search(results = None):
    ...
    if groupbydomain:
        ...
        for r in response['response']['docs']:
            ...
            if highlight: result['highlight'] = highlight
            results.append(result)
    return render_template('search/results.html', query=query, link_query=link_query, display_query=display_query, results=results, no_of_results_for_display=no_of_results_for_display, pagination=pagination)
